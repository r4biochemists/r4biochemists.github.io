...
2.  Another R built-in dataset is called *DNase* and it contains data of several experiments of DNase treatment. Explore data and use the `report` [package documentation](https://easystats.github.io/report/) and use it to test correlation between the DNase variables `conc` and `density`.
...
Creating you own function will be very useful for automation of repetitive analyses or to encapsulate a sequence of expressions that need to be executed numerous times, perhaps under slightly different conditions. Functions are used in most programming languages as separated instructions for specific tasks.Breaking out our code into functions has several advantages:
...
-   It helps us separate work for a team, assigning each independent function to a member of the team.
