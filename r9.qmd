---
title: "R8: R for Molecular Biology"
author: "Modesto"
date: "August 17, 2023"
...
In the code below, we are going to use some simple regular expressions to explore and analyze the data in a multifasta file containing the whole proteome of the bacteria *Bacillus thuringiensis HER1410* ([HER1410.fasta](data/HER1410.fasta)) in our data folder, using `grep()`, `substr()`, `sub()`, and `regexpr()`.
...
Finally, as in the example below with HER1410 genome sequence ([HER1410nt.fasta](data/HER1410nt.fasta)), you can also play with a nucleotide sequence and obtain some useful data, like the `GC()` content, the encoded proteins or the reverse complementary sequence with `rev()` and `comp()`, respectively.
...
pol <- query("pol","AC=P0DPS1",verbose=TRUE,invisible=FALSE)
